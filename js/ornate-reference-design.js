...
(function(){
  ...
  function controls(){
    ...
    parent.querySelectorAll('.design-carousel-controls,.design-carousel-dots,.night-carousel-controls,.night-carousel-dots').forEach(el=>el.style.display='none');
    ...
    dots.innerHTML=Array.from({length:total},(_,i)=>`<button class="ornate-carousel-dot" data-dot="${i}"></button>`).join('');let active=Math.min(Number(window.__prayerActiveDesignIndex)||0,total-1);
    function render(){active=(active+total)%total;track.style.transform=`translateX(-${active*100}%)`;window.__prayerActiveDesignIndex=active;c.querySelector('.ornate-carousel-status').textContent=`التصميم ${active+1} من ${total}`;dots.querySelectorAll('[data-dot]').forEach((x,i)=>x.classList.toggle('active',i===active));window.__prayerActiveDesignElement=slides[active]?.firstElementChild||null;window.dispatchEvent(new CustomEvent('prayerDesignChanged',{detail:{index:active,design:window.__prayerActiveDesignElement}}));resize()}
