...
(function(){
  ...
  function activeDesign(){
    ...
    if(track){
      ...
      const d=slides[Math.max(0,Math.min(index,slides.length-1))]?.firstElementChild;if(d)return d;
    }
  ...
  const prayerRowSelector='.prayer-row,.sd2-prayer-row,.sd3-prayer-row,.sd4-prayer-row,.nd-prayer-row,.nf-row,.ref-prayer-row,[data-prayer-row]';
  ...
  function roleTargets(role){
    ...
    if(prayerLabels[role]){
      ...
      direct.forEach(el=>{const row=el.closest(prayerRowSelector);if(row)rows.push(row);});
      if(!rows.length)Array.from(root.querySelectorAll(prayerRowSelector)).forEach(row=>{if((row.textContent||'').includes(prayerLabels[role]))rows.push(row);});
      const out=[];
      rows.forEach(row=>{
        out.push(row);
      ...
      direct.forEach(el=>out.push(el));return unique(out);
    }
  ...
  function announceChange(){window.dispatchEvent(new CustomEvent('prayerFontChanged',{detail:{design:activeDesign()}}));}
  ...
  function buildColorPalette(){
    ...
    const box=document.createElement('div');box.id='designColorPalette';
    ...
    const render=()=>{const sw=box.querySelector('.design-color-swatches');sw.innerHTML='';collectDesignColors().forEach(color=>{const x=document.createElement('button');x.type='button';x.className='design-color-swatch';x.style.background=color;x.title=`استخدام ${color}`;x.setAttribute('aria-label',`استخدام اللون ${color}`);x.addEventListener('click',()=>applyColor(color));sw.appendChild(x);});};
  ...
  function bind(){
    ...
    select.dataset.universalFontBound='1';select.addEventListener('change',()=>setTimeout(load,0));
    ...
    const color=document.getElementById('fontColor');if(color&&!color.dataset.universalColorBound){color.dataset.universalColorBound='1';color.addEventListener('input',()=>applyColor(color.value));color.addEventListener('change',()=>applyColor(color.value));}
    document.addEventListener('click',e=>{if(e.target.closest('.design-carousel-btn,.design-carousel-dot'))setTimeout(load,100);});
    window.addEventListener('prayerDesignChanged',()=>setTimeout(load,0));buildColorPalette();
  }
